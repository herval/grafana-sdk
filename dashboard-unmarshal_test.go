...
func TestUnmarshal_DashboardWithMixedYaxes(t *testing.T) {
	...
	if min1.Value != 0 || min1.Valid != true {
		t.Errorf("panel #1 has wrong min value: %v, expected: %v", min1.Value, 0)
	}
	if max1.Value != 100 || max1.Valid != true {
		t.Errorf("panel #1 has wrong max value: %v, expected: %v", max1.Value, 100)
	}
	...
	if min3.Value != 0 || min3.Valid != true {
		t.Errorf("panel #2 has wrong min value: %v, expected: %v", min3.Value, 0)
	}
	if max3 != nil {
	...
	if max4.Value != 50 || max4.Valid != true {
		t.Errorf("panel #1 has wrong max value: %v, expected: %v", max4.Value, 100)
	}
}
